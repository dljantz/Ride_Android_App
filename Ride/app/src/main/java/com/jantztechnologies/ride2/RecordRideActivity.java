...
public class RecordRideActivity extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        if (RecordRideService.isServiceCreated()) {
            ...
            // Additionally check if ride2 is recording so we know how to lay out buttons.
            // Below: service is running, ride2 is paused. Resume and Finish buttons both visible
            if (RecordRideService.getIsManuallyPaused()) {
                autoPauseTextView.setVisibility(View.VISIBLE);
            ...
            } else { // ride2 is recording (not paused). PAUSE button visible only.
    ...
    public void onStartResumeButtonClick(View view) {
        // you are starting the ride2 from zero
        if (!isServiceBound && startResumeButt.getText().equals(getResources().getString(R.string.start_button))) {
            ...
            stopLocationRequests();
            handler.removeCallbacks(preRideRunnable); // don't want the button graying out during the ride2
    ...
    public void onStopFinishButtonClick(View view) {
        ...
        // if you are finishing the ride2
        else {
            handler.removeCallbacks(duringRideRunnable);

            // returned boolean makes sure the service has finished saving ride2 before killing it
            String rideFolder = service.saveRide();
            unbindAndEndService();
            Toast.makeText(this, "Ride saved", Toast.LENGTH_LONG).show();

            // pull up activity with ride2 data and map
