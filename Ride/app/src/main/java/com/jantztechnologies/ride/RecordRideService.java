...
import static com.jantztechnologies.ride.App.CHANNEL_ID;
...
public class RecordRideService extends Service implements SensorEventListener {
    ...
    public void onCreate() {
        ...
        notificationManager = NotificationManagerCompat.from(this); // used to update the notification with ongoing ride data
    ...
    private void updateAcceptedLocations(SerializableLocation newLocation) {
        ...
        if (acceptedLocations.size() == 0 && newLocation.accuracyMeters < desiredStartingLocationAccuracyMeters) {
            ...
            // confirm this location and add it to the list. Then update ride distance, speed at this spot, elevation
            if (distanceToNewLocation > farEnoughAway) {
                ...
                //      set up right. Otherwise we get weird stuff like measuring the distance traveled while the ride was manually paused,
    ...
    private boolean updateRideDistanceMeters(SerializableLocation newLocation, double distanceToNewLocation) {
        ...
        // if the ride has never been paused, go ahead and measure distance
    ...
    //      after that ride so they will never know...
    ...
    private void updateElevationStats(SerializableLocation location) {
        ...
        //      last location to this one was while the ride was paused.
    ...
    // saves ride to file, generating a new folder per ride
